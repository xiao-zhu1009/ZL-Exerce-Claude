  ...
  <div class="workout-log">
    ...
    >
            ...
            <el-table :data="row.extra.exercises" size="mini" border>
              ...
              <el-table-column label="组数×次数" width="100">
                <template slot-scope="{ row: ex }"
                  >{{ ex.sets }}组×{{ ex.reps }}次</template
                >
              </el-table-column>
              <el-table-column label="重量" width="90">
                <template slot-scope="{ row: ex }">
                  <span v-if="ex.weight"
                    >{{ ex.weight }}{{ ex.unit || "kg" }}</span
                  >
                  <span v-else>—</span>
                </template>
              </el-table-column>
      ...
      <el-table-column prop="workout_type" label="类型" width="100">
        <template slot-scope="{ row }">
          <el-tag size="small" :type="typeColor(row.workout_type)">{{
            row.workout_type
          }}</el-tag>
        </template>
      </el-table-column>
                ...
                >
                  <template slot-scope="{ item }">
                    ...
                    <span class="action-tag"
                      >{{ item.body_part }} · {{ item.category }}</span
                    >
                  </template>
                </el-autocomplete>
              </el-col>
...
export default {
  ...
  methods: {
    async fetchRecords() {
      this.loading = true;
      try {
        const [start, end] = Array.isArray(this.dateRange)
          ? this.dateRange
          : [];
    ...
    hasDetail(row) {
      return (
        row.extra &&
        row.extra.mode === "detail" &&
        Array.isArray(row.extra.exercises) &&
        row.extra.exercises.length > 0
      );
    },
  },
};
