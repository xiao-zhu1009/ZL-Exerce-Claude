...
export default {
  ...
  methods: {
    applyRouteTab() {
      if ((this.$route.query.tab || '').toString() === 'record') {
    ...
    async submitQuick() {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          const panel = this.$refs.recordPanel
          if (panel && typeof panel.fetchRecords === 'function' && panel.date === today) {
            panel.fetchRecords()
          }
        } else {
